...
Based on https://gitcode.net/mirrors/wavefrontshaping/complexPyTorch
"""
...
from model.complexFunctions import complex_relu, complex_max_pool2d, complex_avg_pool2d, complex_max_pool1d, complex_avg_pool1d
from model.complexFunctions import complex_dropout, complex_dropout2d
...
def apply_complex(fr, fi, input, dtype=torch.complex64):
    ...
    C = fr(input.imag)
    D = fi(input.real)
    ret = (A - B).type(dtype) + 1j * (C + D).type(dtype)
    return ret
...
class ComplexAvgPool1d(Module):
    ...
    def forward(self, input):
        return complex_avg_pool1d(input, kernel_size=self.kernel_size,
                                  stride=self.stride, padding=self.padding,
                                  dilation=self.dilation, ceil_mode=self.ceil_mode,
                                  return_indices=self.return_indices)
